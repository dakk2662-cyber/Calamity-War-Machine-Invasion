...
namespace CalamityAddon.Content.NPCs.WulfrumJumper
{
    public class WulfrumJumperP1 : ModNPC
    {
        ...
        private bool isDespawnJumping;
        private Vector2 despawnDirection;
        private const float DespawnJumpStrength = -18f;
        private const float DespawnHorizontalSpeed = 10f;
        ...
        public override void AI()
        {
            ...
            if (isDespawnJumping)
            {
                ...
                ContinueDespawnJump(player);
                return;
            }

            if (NPC.life <= 1)
            {
                BeginDespawnJump(player);
                return;
            }
        ...
        private void BeginDespawnJump(Player player)
        {
            isDespawnJumping = true;
            NPC.dontTakeDamage = true;
            despawnDirection = NPC.Center - player.Center;
            if (despawnDirection.LengthSquared() < 0.01f)
            {
                despawnDirection = new Vector2(NPC.direction, 0f);
                if (despawnDirection.X == 0f)
                {
                    despawnDirection.X = 1f;
                }
            }
            despawnDirection.Normalize();
            NPC.velocity = new Vector2(despawnDirection.X * DespawnHorizontalSpeed, DespawnJumpStrength);
            NPC.ai[0] = 0;
            NPC.ai[1] = 0;
            NPC.ai[2] = 0;
            NPC.ai[3] = 0;
        ...
        private void ContinueDespawnJump(Player player)
        {
            Vector2 nextDirection = NPC.Center - player.Center;
            if (nextDirection.LengthSquared() < 0.01f)
            {
                nextDirection = new Vector2(NPC.direction, 0f);
            }
            nextDirection.Normalize();
            float targetHorizontal = nextDirection.X * DespawnHorizontalSpeed;
            NPC.velocity.X = MathHelper.Lerp(NPC.velocity.X, targetHorizontal, 0.06f);
            NPC.spriteDirection = nextDirection.X > 0f ? -1 : 1;
        ...
        private void ShootHomingRocket()
        {
            ...
            Vector2 offset = new Vector2(-NPC.direction * 30f, -20f);
